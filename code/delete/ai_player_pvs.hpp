...
// 0 なら全手フルウィンドウ(=提案4版 ai_player_killer.hpp と同一挙動。ノード数完全一致の検証用)
// (実装計画書 implementation-plan-pvs.md §1.2, §5.1-3)
#ifndef USE_PVS
#define USE_PVS 1
#endif

// 提案5(PVS: Principal Variation Search)版。ai_player_killer.hpp の
// AIPlayerKiller(提案1+2+3+4 実装済み)のコピーで、差分は
//   - 内部ノード: pv_done フラグを導入し、2手目以降を null window (alpha, alpha+1) で探索、
//     fail high (alpha < ev < beta) のときのみフルウィンドウで再探索(§3.1)
//   - root: i>0 の手を親窓 (mx-1, mx+1) の null window で探索、ev >= mx+1 のときのみ
//     親窓 (mx, INF) で確定。同点手の全収集+ランダム選択仕様は維持(§3.2)
// (実装計画書 implementation-plan-pvs.md §3。基準は main_alpha_killer.cpp)
// ※ ply の付番は提案4実装に合わせ root からの呼び出しを 0 とする(killer 計画書 §3.1。
//   -DUSE_PVS=0 で基準版とノード数完全一致させるため、計画書 §3.2 例示の 1 ではなく 0)
template<typename F>
struct AIPlayerPVS : Player
{
	...
	AIPlayerPVS(int level, F evaluate_func, int tt_bits = 22) : level(level), evaluate_func(evaluate_func), tt(tt_bits) { assert(level >= 1); }
