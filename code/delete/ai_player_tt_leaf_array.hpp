...
// 提案3(vector → 固定長配列)版。ai_player_tt_leaf.hpp の AIPlayerTTLeaf(提案1+2 実装済み)の
// コピーで、差分は動的 ordering の vector を array<pair<int,ull>,16> + 要素数 n に置換した 2 箇所のみ
// (実装計画書 implementation-plan-fixed-array-ordering.md §3。基準を main_alpha_tt_leaf.cpp に読み替え)
template<typename F>
struct AIPlayerTTLeafArray : Player
{
	...
	AIPlayerTTLeafArray(int level, F evaluate_func, int tt_bits = 22) : level(level), evaluate_func(evaluate_func), tt(tt_bits) { assert(level >= 1); }
