...
// 提案1+2+3(TT+葉reach再利用+固定長配列 ordering)版ハーネス。基準は main_alpha_tt_leaf.cpp
//   g++ -std=c++17 -O2 -fopenmp code/main_alpha_tt_leaf_array.cpp -o yonmoku_alpha_tt_leaf_array
// ノード数計測: -DBENCH

#include "tt.hpp"
#include "ai_player_tt_leaf_array.hpp"
#include "evaluate_alpha_leaf.hpp"
using AI = AIPlayerTTLeafArray<int(*)(const Board&, unsigned long long, unsigned long long, unsigned long long)>;
...
int main()
{
	...
	Game game(&p1, &p2, true, {{3,3}});  // main_alpha_tt_leaf.cpp と同一
	p1.set_game(&game);
	p2.set_game(&game);
	game.game();
