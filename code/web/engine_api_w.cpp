// ===== 立体四目並べ Web 版 エンジン API(重み外部ファイル化版 _w)=====
// 設計: docs/設計書/Web公開/implementation-plan-web-ui-3d.md §3
//       docs/設計書/評価関数バージョン管理/implementation-plan-eval-weights-file.md §3.2
//
// engine_api.cpp との差分は次の 3 点だけで、API・難易度プロファイル・状態遷移は完全に同一。
//   1. include を evaluate_alpha_inc_tbl_w.hpp に差し替え
//   2. using AI を AIPlayerPVSIncID<EvalFn> に差し替え
//   3. init_cnt_tbl() → init_eval_tbl()
// ★WASM はファイルを読めないので、重みは常に組み込み既定値 EvalWeights::builtin() を使う。
//   よって公開中の Web 版の挙動は完全に不変。
//
...
// WASM ではファイルを読めないため、重みは常に組み込み既定値。
static inline const EvalWeights& W() { return EvalWeights::builtin(); }
...
static void ensure_ai(int profile)
{
	...
	if (!g_ai || g_tt_bits != p.tt_bits)
	{
		...
		g_ai = new AI(p.n_open, EvalFn{&W(), false}, p.tt_bits);
		g_tt_bits = p.tt_bits;
	...
	g_ai->evaluate_func = EvalFn{&W(), g_human_is_black};
}
