...
//   g++ -std=c++17 -O2 -fopenmp code/main_core.cpp -o main_core
//   実行時にモデルを差し替える: ./main_core --weights weights/core/verX_Y.txt   (version 1 = weights/alpha/ / version 2 = weights/core/ どちらも可)
...
#include "evaluate_core.hpp"              // ★既存評価(重み外部化版) + 核 2 マスの全状態
using AI = AIPlayerPVSIncID<EvalFnCore>;  // ★EvalFn → EvalFnCore
...
static EvalWeightsCore g_loaded;
static const EvalWeightsCore* g_weights = nullptr;
...
int main(int argc, char** argv)
{
	...
	g_weights = &EvalWeightsCore::builtin();
	...
	cout << "weights: " << g_weights->base.name
	     << (g_weights == &EvalWeightsCore::builtin() ? " (builtin)" : " (file)")
	...
	AI p1(10, EvalFnCore{g_weights, false});   // 先手 = AI
	AI p2(10, EvalFnCore{g_weights, true });   // 後手 = AI
	...
	# pragma omp parallel private(rng)
	{
		...
		{
			AI p1(8, EvalFnCore{g_weights, false});
			AI p2(8, EvalFnCore{g_weights, true });
			p1.set_random(15);
