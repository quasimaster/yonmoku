...
// 既存 ai_player.hpp の AIPlayer に置換表(tt.hpp)を組み込んだもの。
// 探索ロジック・move ordering・評価関数呼び出しは既存のまま(実装計画書 §4)
template<typename F>
struct AIPlayerTT : Player
{
	...
	AIPlayerTT(int level, F evaluate_func, int tt_bits = 22) : level(level), evaluate_func(evaluate_func), tt(tt_bits) { assert(level >= 1); }
	...
	int evaluate_board(Board board, int level, int alpha, int beta)
	{
		...
		{//reach (既存のまま。即勝ち/即負けの return は格納しない)
			...
			}
			{
				...
				{
					const int ev = evaluate_func(board);
#if TT_STORE_LEAF
