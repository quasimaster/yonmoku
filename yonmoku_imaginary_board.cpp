...
struct Board
{
	...
	array<int, LINES_NUM> count(unsigned long long imageMe, unsigned long long imageYou) const
	{
		array<int, LINES_NUM> ret;
		for (int i = 0; i < LINES_NUM; i++)
		{
			if ((imageMe & LINES[i]) && (imageYou & LINES[i])) ret[i] = 0;
			else if (imageMe & LINES[i]) ret[i] = __builtin_popcountll(imageMe & LINES[i]);
			else ret[i] = - __builtin_popcountll(imageYou & LINES[i]);
		}
		return ret;
	}
...
int main()
{
	...
	auto get_imageMe = [&](const Board &board, const unsigned long long rMe, const unsigned long long rYou, 
		const unsigned long long tMe, const unsigned long long tYou) -> unsigned long long //imageMeを求める
	{
		unsigned long long imageMe = 0uLL;
		{//リーチに対して仮想石をおく
            imageMe |= rMe >> SIZE * SIZE;//自分の決勝点の下が自分の仮想石
            imageMe |= rYou;//相手の決勝点が自分の仮想石
        }
		{//makeTに対して仮想石を置く
			imageMe |= tMe >> SIZE * SIZE;//自分のmakeT点の下が自分の仮想石
            imageMe |= tYou;//相手のmakeT点が自分の仮想石
		}
        {//無効な仮想石の削除
			unsigned long long cancel = imageMe & (imageMe >> SIZE * SIZE);
			cancel |= cancel << SIZE * SIZE;
			imageMe &= ~cancel;//imageMeからcancelで立っているビットを取り除く
        }
		{//リーチに対して仮想石をおく もう一回(リーチは2機ある)
            imageMe |= rMe >> SIZE * SIZE;//自分の決勝点の下が自分の仮想石
            imageMe |= rYou;//相手の決勝点が自分の仮想石
        }
		{//無効な仮想石の削除
	...
	auto get_imageYou = [&](const Board &board, unsigned long long rMe, unsigned long long rYou, 
		unsigned long long tMe, unsigned long long tYou) -> unsigned long long //imageYouを求める
	{
		unsigned long long imageYou = 0uLL;
		{//リーチに対して仮想石をおく
            imageYou |= rYou >> SIZE * SIZE;//相手の決勝点の下が相手の仮想石
            imageYou |= rMe;//自分の決勝点が相手の仮想石
        }
		{//makeTに対して仮想石を置く
            imageYou |= tYou >> SIZE * SIZE;//相手のmakeT点の下が相手の仮想石
            imageYou |= tMe;//自分のmakeT点が相手の仮想石
		}
        {//無効な仮想石の削除
			unsigned long long cancel = imageYou & (imageYou >> SIZE * SIZE);
			cancel |= cancel << SIZE * SIZE;
			imageYou &= ~cancel;//imageMeからcancelで立っているビットを取り除く
        }
		{//リーチに対して仮想石をおく もう一回(リーチは2機ある)
            imageYou |= rYou >> SIZE * SIZE;//相手の決勝点の下が相手の仮想石
            imageYou |= rMe;//自分の決勝点が相手の仮想石
	...
	auto evaluate_pointfir = [&](const Board &board, unsigned long long rMe, unsigned long long rYou, 
		unsigned long long tMe, unsigned long long tYou) -> int
	{
		const int turn = board.turn();// turn number (the number of stones = turn - 1)
		if(turn >= 60){
			return 0;
        ...
        unsigned long long imageMe = get_imageMe(board, rMe, rYou, tMe, tYou);//自分の仮想石(現実の石と仮想石を合わせた物)
        unsigned long long imageYou = get_imageYou(board, rMe, rYou, tMe, tYou);//相手の仮想石

		unsigned long long imageReachMe = Board::reach(imageMe) & ~imageYou;//仮想ボードの自分のリーチ
		unsigned long long imageReachYou = Board::reach(imageYou) & ~imageMe;//仮想ボードの相手のリーチ

		static const unsigned long long imageReachMe_tmp = imageReachMe;
		static const unsigned long long imageReachYou_tmp = imageReachYou;
		static const unsigned long long imageReachMe_esc = imageReachMe & ~(imageReachYou << SIZE * SIZE);//下に相手のリーチがある場合を削除
		static const unsigned long long imageReachYou_esc = imageReachYou & ~(imageReachMe_tmp << SIZE * SIZE);//下に相手のリーチがある場合を削除
		imageReachMe = imageReachMe_esc;
		imageReachYou = imageReachYou_esc;

		unsigned long long imageMakeTMe = 0uLL;
		unsigned long long imageMakeTYou = 0uLL;

		auto count = board.count(imageMe, imageYou);
		{//仮想ボードのmakeT点を求める
			for(int i = 0; i < LINES_NUM; i++)
			{
				if(count[i] == 2)
				{
					unsigned long long two = ~imageMe & LINES[i];//LINE内の玉が入っていない部分
					unsigned long long floatthree = two & mask_3 & ~((imageReachYou_tmp | imageMe | imageYou) << SIZE * SIZE);//twoのうち、浮き3段目決勝点の候補
					if(__builtin_popcountll(floatthree) == 1){
						imageMakeTMe |= (two & ~floatthree) & ~(imageReachYou_tmp << SIZE * SIZE) & ~(imageReachYou_tmp >> SIZE * SIZE);
					}
					else if(__builtin_popcountll(floatthree) == 2){
						imageMakeTMe |= floatthree & ~(imageReachYou_tmp << SIZE * SIZE) & ~(imageReachYou_tmp >> SIZE * SIZE);
					}
				}
				else if(count[i] == -2)
				{
					unsigned long long two = ~imageYou & LINES[i];
					unsigned long long floatthree = two & mask_3 & ~((imageReachMe_tmp | imageMe | imageYou) << SIZE * SIZE);
					if(__builtin_popcountll(floatthree) == 1){
						imageMakeTYou |= two & ~floatthree & ~(imageReachMe_tmp << SIZE * SIZE) & ~(imageReachMe_tmp >> SIZE * SIZE);
					}
					else if(__builtin_popcountll(floatthree) == 2){
						imageMakeTYou |= floatthree & ~(imageReachMe_tmp << SIZE * SIZE) & ~(imageReachMe_tmp >> SIZE * SIZE);
					}
				}
			}
		}
		...
		static const int stdweight[36] = {
			-881,892,-43,-15,0,20,38,757,-786,
			-70,-442,-25,0,0,3,27,-729,-38,
			-61,262,-3,21,0,-17,3,790,35,
			-12,1237,0,11,0,-9,-3,1168,33
		};
		
		const int turn_bucket = (turn - 4) / 14;
		for(int i = 0; i < LINES_NUM; i++){
			if(count[i] == 4 && (__builtin_popcountll((LINES[i] >> SIZE * SIZE) & imageYou) != 4)){//自分の玉が4個の時の基礎点の計算
				sum += stdweight[turn_bucket * 9 + count[i] + 4];
			}
			else if(count[i] == -4 && (__builtin_popcountll((LINES[i] >> SIZE * SIZE) & imageMe) != 4)){//相手の玉が4個の時の基礎点の計算
				sum += stdweight[turn_bucket * 9 + count[i] + 4];
			}
			else if(count[i] <= 2 && count[i] >= -2){//玉が1,2個の時の基礎点の計算
				sum += stdweight[turn_bucket * 9 + count[i] + 4];
			}
		...
		// };
		static const int maketweight[32] = {
			-163,738,-1132,-972,1169,-1237,167,1046,
			323,1271,570,-1194,-1083,-602,232,193,
			-1041,-912,-791,-94,0,974,25,151,
			-889,-923,-118,518,-1047,702,-234,-603
		};

		{//makeT点の評価値計算
			sum += __builtin_popcountll(imageMakeTMe & mask_1) * maketweight[turn_bucket * 8 + 0];
			sum += __builtin_popcountll(imageMakeTMe & mask_2) * maketweight[turn_bucket * 8 + 1];
			sum += __builtin_popcountll(imageMakeTMe & mask_3) * maketweight[turn_bucket * 8 + 2];
			sum += __builtin_popcountll(imageMakeTMe & mask_4) * maketweight[turn_bucket * 8 + 3];

			sum -= __builtin_popcountll(imageMakeTYou & mask_1) * maketweight[turn_bucket * 8 + 4];
			sum -= __builtin_popcountll(imageMakeTYou & mask_2) * maketweight[turn_bucket * 8 + 5];
			sum -= __builtin_popcountll(imageMakeTYou & mask_3) * maketweight[turn_bucket * 8 + 6];
			sum -= __builtin_popcountll(imageMakeTYou & mask_4) * maketweight[turn_bucket * 8 + 7];
		}
		
        return sum;

	};
	auto evaluate_pointsec = [&](const Board &board, unsigned long long rMe, unsigned long long rYou, 
		unsigned long long tMe, unsigned long long tYou) -> int
	{
		const int turn = board.turn();// turn number (the number of stones = turn - 1)
		if(turn >= 60){
	...
	auto continuous_fir = [&](const Board &board, unsigned long long rMe, unsigned long long rYou, 
		unsigned long long tMe, unsigned long long tYou) -> int
	{
		const int turn = board.turn();// turn number (the number of stones = turn - 1)
		if(turn >= 60){
		...
		static const int parameter[4] = {349,616,426,903};
		//assert(turn < 64);
		return __builtin_popcountll(imageReachMe & imageReachMe << SIZE * SIZE) * parameter[(turn - 4) / 14];
	};
	auto continuous_sec = [&](const Board &board, unsigned long long rMe, unsigned long long rYou, 
		unsigned long long tMe, unsigned long long tYou) -> int
	{
		const int turn = board.turn();// turn number (the number of stones = turn - 1)
		if(turn >= 60){
		...
		return __builtin_popcountll(imageReachMe & imageReachMe << SIZE * SIZE) * parameter[(turn - 5) / 14];
	};
	auto reach_layer_intersection = [&](const Board &board, const enum Color now, unsigned long long rMe, unsigned long long rYou,
		unsigned long long tMe, unsigned long long tYou, const unsigned long long hand) -> int
	{		
		const int turn = board.turn();// turn number (the number of stones = turn - 1)
		...
		imageReachMe &= ~(((imageMe | imageYou) << SIZE * SIZE) & mask_1);//浮いているリーチのみを求める
		imageReachYou &= ~(((imageMe | imageYou) << SIZE * SIZE) & mask_1);//浮いているリーチのみを求める

		const unsigned long long intersection_3 = (imageReachMe & imageReachYou) & mask_3;
		imageReachMe ^= intersection_3;
		imageReachYou ^= intersection_3;
        ...
        // };
		static const int weightfir[40] = {
			-648,1002,-423,-247,538,86,-739,-444,68,-1045,
			-564,1107,268,-868,404,-1026,164,-832,-1295,1212,
			-481,-324,327,-835,-161,-819,790,-882,763,-760,
			750,73,-158,-131,125,-113,238,982,-243,854
		};
		...
		{//first (black) player
			{//Me, first (black) player
				sum += __builtin_popcountll(imageReachMe & mask_2) * weightfir[((turn - 4) / 14)*10 + 0];//2nd layer_intersection_fir
				sum += __builtin_popcountll(imageReachMe & mask_3) * weightfir[((turn - 4) / 14)*10 + 1];//3rd layer_intersection_fir
				sum += __builtin_popcountll(imageReachMe & mask_4) * weightfir[((turn - 4) / 14)*10 + 2];//4th layer_intersection_fir
			}
			{//You, second (white) player
				sum -= __builtin_popcountll(imageReachYou & mask_2) * weightfir[((turn - 4) / 14)*10 + 3];//2nd layer_intersection_fir
				sum -= __builtin_popcountll(imageReachYou & mask_3) * weightfir[((turn - 4) / 14)*10 + 4];//3rd layer_intersection_fir
				sum -= __builtin_popcountll(imageReachYou & mask_4) * weightfir[((turn - 4) / 14)*10 + 5];//4th layer_intersection_fir
			}
			if (intersection_3)
		...
		{//second (white) player
			{//Me, second (white) player
				sum += __builtin_popcountll(imageReachMe & mask_2) * weightsec[((turn - 5) / 14)*10 + 0];//2nd layer_intersection_sec
				sum += __builtin_popcountll(imageReachMe & mask_3) * weightsec[((turn - 5) / 14)*10 + 1];//3rd layer_intersection_sec
				sum += __builtin_popcountll(imageReachMe & mask_4) * weightsec[((turn - 5) / 14)*10 + 2];//4th layer_intersection_sec
			}
			{//You, first (black) player
				sum -= __builtin_popcountll(imageReachYou & mask_2) * weightsec[((turn - 5) / 14)*10 + 3];//2nd layer_intersection_sec
				sum -= __builtin_popcountll(imageReachYou & mask_3) * weightsec[((turn - 5) / 14)*10 + 4];//3rd layer_intersection_sec
				sum -= __builtin_popcountll(imageReachYou & mask_4) * weightsec[((turn - 5) / 14)*10 + 5];//4th layer_intersection_sec
			}
			if (intersection_3)
			{//if there exists intersections of reaches on 3rd layer
				int intersection = __builtin_popcountll(intersection_3);
	...
	auto evaluate_pointfir_cont_layer_intersection = [&](const Board &board) -> int
	{
		...
		static const unsigned long long mask_1 = 0x000000000000ffffuLL;
		static const unsigned long long mask_3 = 0x0000ffff00000000uLL;
		{//浮きmakeTを求める。浮いていないmakeT点に対して仮想石を置くと、現実の石と仮想石が同じ場所に2色存在してしまう場合がある。
			...
			for(int i = 0; i < LINES_NUM; i++){
				if(LINES[i] == 2){
					...
					if(__builtin_popcountll(floatthree) == 1){
						tMe |= (two & ~floatthree) & ~((board.Me | board.You) << SIZE * SIZE) & ~mask_1;
					}
					else if(__builtin_popcountll(floatthree) == 2){
						tMe |= floatthree & ~((board.Me | board.You) << SIZE * SIZE);
					}
				}
				else if(LINES[i] == -2){
					...
					if(__builtin_popcountll(floatthree) == 1){
						tYou |= two & ~floatthree & ~((board.Me | board.You) << SIZE * SIZE) & ~mask_1;
					}
					else if(__builtin_popcountll(floatthree) == 2){
						tYou |= floatthree & ~((board.Me | board.You) << SIZE * SIZE);
					}
				}
			}
		}

		return evaluate_pointfir(board, rMe, rYou, tMe, tYou) + continuous_fir(board, rMe, rYou, tMe, tYou) 
		- continuous_fir(board, rYou, rMe, tYou, tMe) 
		+ reach_layer_intersection(board,now, rMe, rYou, tMe, tYou, hand);
	};
	auto evaluate_pointsec_cont_layer_intersection = [&](const Board &board) -> int
	{
		...
		return evaluate_pointsec(board, rMe, rYou, tMe, tYou) + continuous_sec(board, rMe, rYou, tMe, tYou) 
		- continuous_sec(board, rYou, rMe, tYou, tMe) 
		+ reach_layer_intersection(board,now, rMe, rYou, tMe, tYou, hand);
	};
